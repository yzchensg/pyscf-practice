...
print("HF0_Density = ", HF0_Density.shape)


HF_Fock =  np.load("Fock_matrices_10000.npz")["arr_0"][:5000]
deltas = (HF_Fock - HF0_Density) # kcal/mol/Angstrom
...
print("HF_Fock = ",np.shape(HF_Fock))
print("deltas = ", np.shape(deltas))
print("HF_Fock range = ", np.round(np.min(HF_Fock),3),"->",np.round(np.max(HF_Fock),3))
...
#epsilon = 1e-10
#percent_errors = np.abs(test_predictions - test_labels) / (np.abs(test_labels) + epsilon) * 100
...
### Save model
filename = './saved_data/Fock_1e_model_delta_norm_' + str(np.round(MARD_unormalized,3)) + '%_' + str(min_deltas) + '_max_' + str(max_deltas) + '.h5'
model.save(filename)
sys.exit()
...
# Save the figure
plt.savefig("./saved_data/test_predict_Fock_heatmap_comparison.png")
plt.show()
